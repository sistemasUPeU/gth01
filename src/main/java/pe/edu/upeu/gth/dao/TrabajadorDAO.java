...
public class TrabajadorDAO {
	...
	public List<Map<String,Object>> DATOS_TRABAJADOR(String idtr,String iddepa) {
    	...
    	List<Map<String,Object>> trabajadorMap = new ArrayList<>();
    	List<Map<String,Object>> listonazo = new ArrayList<>();
    	List<Map<String,Object>> jefeMap = new  ArrayList<>();
    	Map<String,Object> map  = new HashMap<>();
    	String sql2="";
    	sql = "RA_SP_TRAER_TRABAJADOR_JASPER";
    	sql2="RA_SP_TRAER_JEFE";

    	try {   
    		SimpleJdbcCall jdbcCall = new SimpleJdbcCall(jt)
    			    .withProcedureName(sql2);

    			jdbcCall.addDeclaredParameter(new SqlParameter("IDDEPA", OracleTypes.VARCHAR));
    			jdbcCall.addDeclaredParameter(new SqlOutParameter("c_jefe", OracleTypes.CURSOR));
    			Map<String, Object> resultado = jdbcCall.execute(iddepa);
    		SimpleJdbcCall jdbcCall2 = new SimpleJdbcCall(jt)
    				    .withProcedureName(sql);

    				jdbcCall2.addDeclaredParameter(new SqlParameter("IDTR", OracleTypes.VARCHAR));
    				jdbcCall2.addDeclaredParameter(new SqlOutParameter("c_trabajador", OracleTypes.CURSOR));
    				Map<String, Object> resultado2 = jdbcCall2.execute(idtr);
    				System.out.println("Mapenado trabajador"+gson.toJson(resultado2));
       
    				System.out.println("Mapeando al jefe" + resultado);
    				trabajadorMap = (List<Map<String,Object>>) resultado2.get("c_trabajador");
    				jefeMap = (List<Map<String,Object>> ) resultado.get("c_jefe");
    			
    			System.out.println("llegado indice 2: "+trabajadorMap.get(0).get("NOMBRES").toString());
    			SimpleDateFormat formatter = new SimpleDateFormat("dd/MMM/yyyy");
//    			String fecha_contrato = formatter.format(liston.get("FECHA_CONTRATO").toString());
    			map.put("FECHA_CONTRATO",trabajadorMap.get(0).get("FECHA_CONTRATO").toString());
    			map.put("NO_TRABAJADOR", trabajadorMap.get(0).get("NOMBRES"));
    			map.put("AP_PATERNO", trabajadorMap.get(0).get("PATERNO"));
    			map.put("AP_MATERNO", trabajadorMap.get(0).get("MATERNO"));
    			map.put("NO_PUESTO", trabajadorMap.get(0).get("NOM_PUESTO")); 
    			map.put("NOM", jefeMap.get(0).get("NOM"));
    			map.put("PAT", jefeMap.get(0).get("PAT"));
    			map.put("MAT", jefeMap.get(0).get("MAT"));
    			listonazo.add(map);
    			System.out.println("map lleno" + gson.toJson(map));    
			
		} catch (Exception e) {
			System.out.println("Esto sale del dao: "+gson.toJson(map));
